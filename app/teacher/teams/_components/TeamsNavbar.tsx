    ...
    <header className="sticky top-0 z-50 glass-panel shadow-sm" style={{ borderBottom: `1px solid ${tx.borderS}` }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16 items-center">
          <button onClick={onBack} className="flex items-center gap-2 font-bold hover:text-indigo-600 dark:hover:text-indigo-400 transition-all duration-200 active:scale-95">
            <ArrowLeft className="h-5 w-5" /> กลับหน้าแดชบอร์ดครู
          </button>
          <div className="flex items-center gap-3">
            <button onClick={toggleDarkMode} className="btn-icon p-2 rounded-xl hover:bg-slate-200/70 dark:hover:bg-slate-700/40" style={{ color: tx.secondary }}>
            ...
            <button onClick={logout} className="btn-icon p-2 rounded-xl text-rose-500 hover:bg-rose-500/10">
