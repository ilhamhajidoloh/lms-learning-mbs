...
import { Search, Video, Award, BookOpen } from "lucide-react";
...
export function CoursesTab({
  ...
  return (
    <div className="space-y-8 animate-fadeIn text-left">
          ...
          {(() => {
            ...
            return levelsList.map(({ code, label }) => (
              <button key={code} onClick={() => setLevelFilter(code)}
                className="px-3.5 py-1.5 rounded-full text-xs font-bold transition-all duration-200 border active:scale-95"
      ...
      {/* Course Search */}
      <div className="relative rounded-2xl border input-glow" style={{ borderColor: tx.border, backgroundColor: tx.surface }}>
        <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 transition-colors duration-200 peer-focus:text-indigo-500" style={{ color: tx.faint }} />
        <input type="text" placeholder="ค้นหาชื่อคอร์สเรียน..." value={search} onChange={(e) => setSearch(e.target.value)}
          className="w-full pl-12 pr-4 py-3.5 bg-transparent border-0 focus:outline-none focus:ring-0 text-sm peer" style={{ color: tx.primary }} />
      </div>
      ...
      {filtered.length === 0 ? (
        <EmptyState
          ...
          description="ไม่มีคอร์สเรียนที่ตรงกับคำค้นหาหรือตัวกรองที่คุณเลือก ลองเปลี่ยนคำค้นหาใหม่ดูนะครับ"
        />
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {filtered.map((course, i) => (
            <div key={course.id} className={`rounded-3xl overflow-hidden shadow-lg flex flex-col justify-between group relative card-hover animate-slideInUp stagger-${Math.min(i + 1, 6)}`} style={{ backgroundColor: tx.surface, border: `1px solid ${tx.borderS}` }}>
              ...
              <div className="p-6 space-y-5">
                ...
                {course.isEnrolled ? (
                  <button onClick={() => {
                    ...
                    className="w-full py-3.5 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white font-extrabold text-xs shadow-lg transition-all hover:-translate-y-0.5 hover:shadow-xl active:scale-95 flex items-center justify-center gap-2">
                    <BookOpen className="h-4 w-4" /> เริ่มเรียนวิชานี้เลย
                  </button>
                ) : course.isOpen ? (
                  <button onClick={() => handleEnroll(course.id, false)}
                    className="w-full py-3.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-extrabold text-xs shadow-lg transition-all hover:-translate-y-0.5 hover:shadow-xl active:scale-95 flex items-center justify-center gap-2">
                    <BookOpen className="h-4 w-4" /> ลงทะเบียนเรียนทันที
                  </button>
                ) : course.enrollCodeRequired ? (
                  <button onClick={() => handleEnroll(course.id, true)}
                    className="w-full py-3.5 rounded-xl bg-purple-600 hover:bg-purple-500 text-white font-extrabold text-xs shadow-lg transition-all hover:-translate-y-0.5 hover:shadow-xl active:scale-95 flex items-center justify-center gap-2">
